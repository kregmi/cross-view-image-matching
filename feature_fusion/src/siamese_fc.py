...
class Siamese_FC:
    ...
    def one_layer_siamese_fc(self, x_sat, x_grd, trainable, scope_name):
        print('Siamese_FC:', scope_name, ' trainable =', trainable)

        with tf.variable_scope(scope_name) as scope:
            fc_sat = self.fc_layer(x_sat, 1000, 1000, 0.005, 0.01, trainable, 'fc1', activation_fn=None)   
            sat_global = tf.nn.l2_normalize(fc_sat, dim=1)

            fc_grd = self.fc_layer(x_grd, 2000, 1000, 0.005, 0.01, trainable, 'fc2', activation_fn=None)
