...
opt = {
   ...
   name = 'cvusa_synthesis_xfork_g2a',              -- name of the experiment, should generally be passed on the command line
   ...
   which_epoch = '50',            -- epoch number to resume training, used only if continue_train=1
...
function defineG(input_nc, output_nc_seg, output_nc, ngf)
    ...
    elseif  opt.which_model_netG == "forkG_cvusa" then netG = defineG_encoder_decoder_fork_cvusa(input_nc, output_nc_seg, output_nc, ngf)
...
----------------------------------------------------------------------------
local real_A = torch.Tensor(opt.batchSize, output_nc, 256, 1024)    -- real image view 2
...
local real_AB = torch.Tensor(opt.batchSize, output_nc + input_nc*opt.condition_GAN, 512, 512) -- real image pairs in two views for D
local fake_AB = torch.Tensor(opt.batchSize, output_nc + input_nc*opt.condition_GAN, 512, 512) -- image pairs in two views for D, one is synthesized
