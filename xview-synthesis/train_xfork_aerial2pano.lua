...
opt = {
   ...
   niter = 30,            -- #  of iter at starting learning rate  -- was 200
   ...
   name = 'cvusa_synthesis_xfork_aerial2pano',              -- name of the experiment, should generally be passed on the command line
   ...
   which_model_netG = 'forkG',  -- selects model to use for netG  crossview_G_64
   which_model_netD = 'basic_D',  -- selects model to use for netG  crossview_G_64
   which_epoch = '25',            -- epoch number to resume training, used only if continue_train=1
   finetune=1,    -- 1 to load weights from encoders and start training, 0 to resume training 
   lambda1=100,
   lambda2=10,
   train_data_path = '/home/kregmi/Documents/cvusa_cvpr2017/crossnet-master/splits/train-19zl-segmap-rgb.csv',  
   use_GAN=1,
   use_L1=1,
}
...
function defineG(input_nc, output_nc_seg, output_nc, ngf)
    ...
    if  opt.which_model_netG == "forkG" then netG = defineG_encoder_decoder_fork_a2p(input_nc, output_nc_seg, output_nc, ngf)
...
----------------------------------------------------------------------------
local real_A = torch.Tensor(opt.batchSize, input_nc, 512, 512)     -- real image view 1
local real_B = torch.Tensor(opt.batchSize, output_nc, 256, 1024)    -- real image view 2
local real_Bs = torch.Tensor(opt.batchSize, output_nc, 256, 1024)    -- real image view 2

local fake_B = torch.Tensor(opt.batchSize, output_nc, 256, 1024)    -- real image view 2
local fake_Bs = torch.Tensor(opt.batchSize, output_nc, 256, 1024)    -- real image view 2



opt.condition_GAN = 0

local fake_img = torch.Tensor(opt.batchSize, output_nc + output_nc_seg, 256, 1024)
local real_AB = torch.Tensor(opt.batchSize, output_nc + input_nc*opt.condition_GAN, 256, 1024) -- real image pairs in two views for D
local fake_AB = torch.Tensor(opt.batchSize, output_nc + input_nc*opt.condition_GAN, 256, 1024) -- image pairs in two views for D, one is synthesized
...
function createImageBatch(n)
  ...
  for i = 1, batchSize do
    local aerial_image, ground_image, ground_seg = load_and_preprocess_regular_image(opt.DATA_ROOT .. '/' .. m[perm_rand[n+i-1]][1], opt.DATA_ROOT .. '/' .. m[perm_rand[n+i-1]][2], opt.DATA_ROOT .. '/' .. m[perm_rand[n+i-1]][3])
    aerial_image = image.scale(aerial_image, 512, 512)
    ground_image = image.scale(ground_image, 1024, 256)
    ground_seg = image.scale(ground_seg, 1024, 256)
    real_A[i] = aerial_image
    real_B[i] = ground_image
    real_Bs[i] = ground_seg
  end
  data_tm:stop()
