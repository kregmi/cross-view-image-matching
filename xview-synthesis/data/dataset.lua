...
local function tableToOutput(self, Ah_Table, Sh_Table, As_Table, Ss_Table, scalarTable)
   local AhLabels, ShLabels, AsLabels, SsLabels, scalarLabels, labels
...
--   print(self.sampleSize[3])
   ...
   AhLabels = torch.Tensor(quantity, 960, 17, 17)
   ShLabels = torch.Tensor(quantity, 960, 8, 40)
   AsLabels = torch.Tensor(quantity, 4, 17, 17)
   SsLabels = torch.Tensor(quantity, 4, 8, 40)
...
--	 print(data:size())
   ...
   -- end
    ...
    for i=1,#Ah_Table do
      AhLabels[i]:copy(Ah_Table[i])
      ShLabels[i]:copy(Sh_Table[i])
      AsLabels[i]:copy(As_Table[i])
      SsLabels[i]:copy(Ss_Table[i])
      scalarLabels[i] = scalarTable[i]
   end
   return AhLabels, ShLabels, AsLabels, SsLabels, scalarLabels
end
...
--      print(class)
      ...
      local input_feats, imgpath = self:getByClass(class)
      -- table.insert(dataTable, out)
      table.insert(Ah_Table, input_feats[1])
      table.insert(Sh_Table, input_feats[2])
      table.insert(As_Table, input_feats[3])
      table.insert(Ss_Table, input_feats[4])
...
--   print(table)
   ...
   local Ah, Sh, As, Ss, scalarLabels = tableToOutput(self, Ah_Table, Sh_Table, As_Table, Ss_Table, scalarTable)
   return Ah, Sh, As, Ss, scalarLabels, samplePaths-- filePaths
end
...
function dataset:get(i1, i2)
   ...
   local AhLabels, ShLabels, AsLabels, SsLabels, scalarLabels = tableToOutput(self, Ah_Table, Sh_Table, As_Table, Ss_Table, scalarTable)
   return Ah_Table, Sh_Table, As_Table, Ss_Table, scalarTable
end
