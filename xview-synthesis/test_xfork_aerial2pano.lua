...
opt = {
   DATA_ROOT = '/home/kregmi/Documents/cvusa_cvpr2017/crossnet-master',         -- path to images (should have subfolders 'train', 'val', etc)
    ...
    name = 'cvusa_synthesis_xfork_aerial2pano',                -- name of experiment, selects which model to run, should generally should be passed on command line
    ...
    train_data_path = '/home/kregmi/Documents/cvusa_cvpr2017/crossnet-master/splits/val-19zl.csv', -- to synthesize images for evaluation of test set
}
...
----------------------------------------------------------------------------
local input = torch.Tensor(opt.batchSize, input_nc, 512, 512)     -- real image view 1
local output_img = torch.Tensor(opt.batchSize, output_nc, 256, 1024)    -- real image view 2
local output_seg = torch.Tensor(opt.batchSize, output_nc, 256, 1024)    -- real image view 2
...
function createImageBatch(n)
  ...
  for i = 1, batchSize do
    ...
    local aerial_image = load_and_preprocess_regular_image(opt.DATA_ROOT .. '/' .. m[n+i-1][1])
    aerial_image = image.scale(aerial_image, 512, 512)
    input[i] = aerial_image
  end
...
for n = 1, number_of_files, batchSize do
    ...
    output_img = deprocess_batch(output[1])
