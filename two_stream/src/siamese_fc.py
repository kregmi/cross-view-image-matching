...
class Siamese_FC:
    ...
    def my_siamese_fc(self, x_sat, x_grd, trainable, scope_name):
        print('Siamese_FC:', scope_name, ' trainable =', trainable)

        with tf.variable_scope(scope_name) as scope:
            fc_sat = self.fc_layer(x_sat, 8*8*512, 4096, 0.005, 0.1, trainable, 'fc1', activation_fn=None)
            sat_global = tf.nn.l2_normalize(fc_sat, dim=1)

 #           scope.reuse_variables()

            fc_grd = self.fc_layer(x_grd, 5*16*512, 4096, 0.005, 0.1, trainable, 'fc2', activation_fn=None)
            grd_global = tf.nn.l2_normalize(fc_grd, dim=1)
    ...
    def my_siamese_fc_multiscale(self, x_sat, x_grd, trainable, scope_name):
        print('Siamese_FC:', scope_name, ' trainable =', trainable)

        with tf.variable_scope(scope_name) as scope:
            fc_sat = self.fc_layer(x_sat, 43008, 1000, 0.005, 0.1, trainable, 'fc1', activation_fn=None)
            sat_global = tf.nn.l2_normalize(fc_sat, dim=1)
