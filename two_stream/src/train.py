from my_net import two_stream_baseline
...
network_type = 'two-stream_baseline'
...
def train(start_epoch=1):
    ...
    # build model
    if network_type == 'two-stream_baseline':
        sat_global, grd_global = two_stream_baseline(sat_x, grd_x, keep_prob, is_training)
    else:
        print ('CONFIG ERROR: wrong network type, only two-stream_baseline is valid')
...
#        print('load model...FINISHED')
        ...
        for epoch in range(start_epoch, start_epoch + number_of_epoch):
            ...
            print('   %d: Test accuracy = %.2f%%' % (epoch, val_accuracy*100.0))
